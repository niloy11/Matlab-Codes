...
for i=1:length(f)
    ...
    k11(i) = km * (kp1*(1+alp1)+ km + f(i)*(kp1*(1-alp1)-km ) )/(kp1*(1+alp1)+ km - f(i)*(kp1*(1-alp1)-km ) );
    k31(i)= (1-f(i))*km + f(i)* kp1;
 
%      ka1(i)=k11(i);
     ka1(i)=k31(i);   % for transverse conductivity
    ke1(i)=ka1(i)*fr1;
   
    k12(i) = km * (kp2*(1+alp2)+ km + f(i)*(kp2*(1-alp2)-km ) )/(kp2*(1+alp2)+ km - f(i)*(kp2*(1-alp2)-km ) );
    k32(i)= (1-f(i))*km + f(i)* kp2;
%     ka2(i)=k12(i);
    ka2(i)=k32(i);   % for transverse conductivity
   ke2(i)=ka2(i)*fr2;
   
    k13(i) = km * (kp3*(1+alp3)+ km + f(i)*(kp3*(1-alp3)-km ) )/(kp3*(1+alp3)+ km - f(i)*(kp3*(1-alp3)-km ) );
    k33(i)= (1-f(i))*km + f(i)* kp3;
%     ka3(i)=k13(i);
    ka3(i)=k33(i);   % for transverse conductivity
   ke3(i)=ka3(i)*fr3;
  
  ktim22(i)=ke1(i)+ke2(i)+ke3(i);
